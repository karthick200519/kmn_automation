...
export const MotorDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadMotor = async () => {
      setLoading(true);
      const motorNumber = id ? parseInt(id, 10) || 1 : 1;
      const data = await motorService.getMotorById(motorNumber);
      setMotor(data);
  ...
  return (
    <MainLayout pageTitle={`Motor ${motor.motor_number} Technical Inspection`}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${getSeverityColorClass(motor.motor_status)}`}>
            Status: {motor.motor_status}
          </span>
          <span className="text-xs font-semibold text-slate-500">
            Last Updated: {formatTimeAgo(motor.sensor_timestamp)}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
          ...
          <p className="text-3xl font-black text-slate-900">{motor.health_index ?? 95} <span className="text-sm font-normal text-slate-500">/ 100</span></p>
          <p className="text-xs font-semibold text-emerald-600">{motor.health_status || 'Optimal'}</p>
        </div>

        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Current Diagnosis</p>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
          ...
          <p className="text-3xl font-black text-blue-600">{Math.round((motor.confidence || 0.95) * 100)}%</p>
          <p className="text-xs text-slate-500">Model Version: {motor.model_version || 'v1.2.0'}</p>
        </div>

        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Severity Level</p>
          <div className="mt-1">
            <span className={`px-2.5 py-1 rounded text-xs font-bold uppercase ${getSeverityColorClass(motor.severity)}`}>
              {motor.severity || 'Low'}
            </span>
          </div>
      ...
      {/* 2. Parameters Breakdown (Electrical, Mechanical, Thermal) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
          <div className="flex items-center space-x-2 border-b border-slate-100 pb-3">
            ...
            <h3 className="font-bold text-slate-900 text-sm">Electrical Parameters</h3>
          </div>
          <div className="grid grid-cols-2 gap-4 text-xs">
            ...
            <div className="p-3 bg-slate-50 rounded-lg">
              <p className="text-slate-500 font-medium">Accumulated Energy</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
          <div className="flex items-center space-x-2 border-b border-slate-100 pb-3">
            ...
            <h3 className="font-bold text-slate-900 text-sm">Predictive Maintenance Decision</h3>
          </div>
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={history}>
              ...
              <XAxis dataKey="timestamp" stroke="#94A3B8" fontSize={11} tickFormatter={(t) => new Date(t).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} />
              <YAxis yAxisId="left" stroke="#3B82F6" fontSize={11} label={{ value: 'Vibration (g)', angle: -90, position: 'insideLeft' }} />
              <YAxis yAxisId="right" orientation="right" stroke="#EF4444" fontSize={11} label={{ value: 'Temp (°C)', angle: 90, position: 'insideRight' }} />
              <Tooltip />
              <Line yAxisId="left" type="monotone" dataKey="vibration_rms" stroke="#3B82F6" strokeWidth={2} dot={false} name="Vibration (g)" />
